import os
import sys
if '/root/my_code/neuralangelo' not in sys.path:
...
from robust.configs.config import OUT_DIR, LOG_DIR
from robust.utils.loss_utils import NANLoss
from robust.dataloaders import dataset_dict
from robust.dataloaders.basic_dataset import de_linearize, Mode
...
from robust.utils.eval_utils import mse2psnr, img2psnr
from robust.utils.general_utils import img_HWC2CHW
from robust.utils.io_utils import print_link, colorize
...
class Trainer:
    def __init__(self, args):
        ...
        # Create NAN scheme
        self.model = RobustModel.create(args)
    ...
    def log_iteration(self, ray_batch_out, ray_batch_in, dt, global_step, epoch):
        ...
        mse_error = L2Loss({}).loss_fn(de_linearize(ray_batch_out['coarse'].rgb, ray_batch_in['white_level']),
        ...
        if ray_batch_out['fine'] is not None:
            mse_error = L2Loss({}).loss_fn(de_linearize(ray_batch_out['fine'].rgb, ray_batch_in['white_level']),
                                de_linearize(ray_batch_in['rgb'], ray_batch_in['white_level'])).item()
